...
% TODO: string type?
The string was tied around a screw, which was inserted into the mass (tennis ball). Two stacks of books stabilized two boards which jutted off the table and clamped the string, fixing the centre of rotation. A tripod supported the camera (a phone) from a distance, capturing slow-motion footage.
% TODO: talk about Q
\subsection{Measuring Period vs. Initial Amplitude}
The period was measured with 3 oscillations per trial to reduce type B uncertainty using frame-by-frame analysis of the footage in the Open Source Physics (OSP) Tracker. A protractor taped to the end of the boards measured initial amplitudes ($\pm \pi/2$). When dropping the mass, it was ensured from another angle that the pendulum would swing on a plane perpendicular to the protractor. The string was clamped close to the protractor to prevent parallax in readings (see Figure~\ref{fig:setup1}). 
...
$Q=nK,$ so $u(Q)=nu(K)$. Smaller $n$ result in a larger number of oscillations for which $\theta\approx\theta_{final}$ experimentally \mbox{$(\,\abs{\theta_{final}-\theta} \leq 2u(\theta))$}, increasing $u(K)$. Data from both $n=2$ and $n=3$ were collected, and the dataset with lower $u(Q)$ was used.


\section{Results and Data Analysis}
\begin{figure}[H]
    \centering
    \includegraphics[height=0.3\textheight]{output-files/period vs amplitude.png}
    \caption{
        Graph of period vs. initial amplitude at release fitted to the 3-term power series 
        \mbox{$T(\theta_0) = T_0(1 + B\theta_0 + C\theta_0^2)$} (Wilson, 2025). 
        Amplitude error bars are too small to be visible.
...
The data fits the power series with values ($B,~ C=0$ predicted (Wilson)):
\begin{align*}
...
However, contrary to prediction, $C \neq 0$ experimentally (considering error $C \gg u(C)$, $C$ cannot plausibly considered 0). Indeed, $T(\theta_0)$ grows non-negligibly for larger $\abs{\theta_0}$.
...
\begin{center}
    ...
    \end{math}
\end{center}
Therefore, the small angle approximation is valid for $\abs{\theta_0}\leq 0.28$. According to the consistent error bars in Figure~\ref{fig:period vs amplitude}, $u(T)=\SI{0.003}{s}$ is indeed valid for $\abs{\theta_0}\leq 0.28~ (u_A(T) < u_B(T))$. However, when $\abs{\theta_0}>0.28$, $T$ depends non-negligibly on $\theta_0$.

\begin{figure}[H]
    \centering
    \includegraphics[height=0.3\textheight]{output-files/amplitude vs. time.png}
    \caption{
        Graph of $\theta$ at the peaks of oscillations vs. $t$ fitted to the exponential
        $\theta(t) = \theta_0 \e ^ {t/\tau}$ (Wilson, 2025). 
        Time error bars are too small to be visible.
    }
    \label{fig:amplitude vs. time}
\end{figure}

The exponential fit values and period:
\begin{align*}
    \theta_0&= 0.271 \pm 0.002, \\
    \tau&= 51.3 \pm \SI{0.5}{s} \\
    T&= \frac{T_{final}+T_0}{2} \pm \frac{T_{final}-T_0}{2} = 1.083 \pm \SI{0.008}{s}
\end{align*}
From these values, 
\begin{displaymath}
    Q=\pi\frac{\tau}{T} 
