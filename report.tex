...
% EXPERIMENT 4
Experiment 4 examined the dependency of \textbf{$Q$ factor vs. length}, calculating $Q$ values with the same method as experiment 2. After fitting the data to many curves (Appendix \ref{trying curves}), $Q=kL^n$ yielded reasonable fit uncertainties and residuals, $k=380\pm\SI{20}{m^{-0.76}}$, $n=0.76 \pm 0.03$.
}
...
The pendulum was dropped from slightly higher than $\theta_0<0.25$ {\color{darkGreen}(small angle approximation found in Experiment 1)} so it would stabilize after any potential human error (e.g. non-zero force) by the time it reached $\theta_0$.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        {\color{darkGreen}Most, but not all $T$ error bars hit the best fit curve, so it is an accurate model with reasonable uncertainties}.}
...
\begin{figure}[H]
    ...
    \caption{
        ...
        {\color{darkGreen}Most, but not all error bars overlap the curve, so uncertainties are reasonable, and the decay can reasonably be modelled with Equation \ref{max amplitude vs. time} but initially underestimates and eventually overestimates $\theta_{\text{max}}$.}
    }
...
As in the $T$ vs. $\theta_0$ analysis, $u(T)=\max(u_A(T), u_B(T))$ where $u_A(T)=$ standard error of the mean of 3 trials, ranging from $0.0006$ to $0.002\,$s, and $u_B(T)=1/\SI{120}{FPS}/\SI{10}{oscillations}=\SI{0.0008}{s}$. It is possible $u_A(T)$, and thus $u(T)$ was underestimated due to insufficient trials. A combination of the metre stick's slight curve, its slight misalignment from the vertical, and parallax error cause at best $u(L)=\SI{0.005}{m}$.
...
{\color{darkGreen}
It was confirmed that $T \propto \sqrt{L}$. Best fit values matched equation \ref{period vs length bad}, but the $T$ vs. $L$ data also matches the theoretical equation \ref{period vs length} (correct units).
}
