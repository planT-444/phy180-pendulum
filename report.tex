...
The pendulum was dropped from slightly higher than $\theta_0<0.25$ (the small angle approximation found in Experiment 1) so it would stabilize after any potential human error (e.g. non-zero force) by the time it reached $\theta_0$.
...
\begin{figure}[H]
    ...
    \caption{
        ...
        Most, but not all $T$ error bars overlap the best fit curve, so it is an accurate model with reasonable uncertainties.}
...
\begin{figure}[H]
    ...
    \caption{
        ...
        Most, but not all error bars overlap the curve, so uncertainties are reasonable. The decay can reasonably be modelled with Equation \ref{max amplitude vs. time} but initially underestimates and eventually overestimates $\theta_{\text{max}}$.
    }
...
\begin{figure}[H]
    \centering
        ...
        \caption{
            ...
            Most, but not all $Q$ error bars overlap with the curve, so it is an accurate model and uncertainties are reasonable.
        }
...
To improve results, more trials would be taken for the outlier in experiment 1 and for all points in experiment 3 as well (for more precise $u_A(T)$, since $u(T)$ was likely underestimated).
