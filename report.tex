...
Footage was collected starting at $\theta_0 \approx \pi/6$. As the pendulum could drift off plane with many oscillations, the mass was now hung from two strings of equal length to prevent motion normal to the desired plane. See Figure~\ref{fig:setup2}. The lens of the phone camera was normal to said plane and level with the centre of rotation to reduce parallax error, since max amplitude was now measured using OSP Tracker (manual tracking). 
...
\end{align*}
As expected, $|B - u(B)| < 2u(B)$ and $B \approx 0$ experimentally, implying a symmetrical pendulum since
$T(\theta)=T(1+C\theta_0^2)$ is an even function.

However, contrary to prediction, $C \neq 0$ experimentally (considering error $C \gg u(C)$, C cannot plausibly considered 0). Indeed, $T(\theta_0)$ grows non-negligibly for larger $|\theta_0|$.
...
To determine what $\theta_0$ allow $T\approx T_0$ (small angle approximation), it must be determined when discrepancy $T-T_0=T_0C\theta_0^2 \leq u(T)$. \\
\begin{center}
    \begin{math}
        |\theta_0|\leq \sqrt{\frac{u(T)}{T_0 C}}
            \pm \sqrt{
                    \max(
                        \frac{u(T_0)}{T_0},~ 
                        \frac{u(C)}{{C}}
                ...
                \sqrt{\frac{u(T)}{T_0 C}}=0.18 \pm 0.02
    \end{math}
\end{center}
Therefore, the small angle approximation is valid for $|\theta_0|\leq 0.20$. By the error bars in Figure~\ref{fig:period vs amplitude}, $u(T)=\SI{0.003}{s}$ is valid for $|\theta_0|\leq 0.20~ (u_A(T) < u_B(T))$.
