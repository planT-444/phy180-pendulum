...
\begin{figure}[H]
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \caption{
            ...
            $n=0.500 \pm 0.001$. No error bars overlap the curve.
        }
...
As in the $T$ vs. $\theta_0$ analysis, $u(T)=\max(u_A(T), u_B(T))$ where $u_A(T)=$ standard error of the mean of 3 trials, ranging from $0.0006$ to $0.002\,$s, and $u_B(T)=1/\SI{120}{FPS}/\SI{10}{oscillations}=\SI{0.0008}{s}$. A combination of the metre stick's slight curve, its slight misalignment from the vertical, and parallax error cause at best $u(L)=\SI{0.005}{m}$. No error bars overlap the fitted curve in Figures \ref{fig:period vs length} and \ref{fig:period vs length logged}, so $u(T)$ was likely underestimated.
