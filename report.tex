...
Footage was collected starting at $\theta_0 \approx 0.26$. As the pendulum could drift off plane with many oscillations, the mass was now hung from two strings of equal length to prevent motion normal to the desired plane (see Figure~\ref{fig:setup2}). The lens of the phone camera was normal to said plane and level with the centre of rotation to reduce parallax error, since max amplitude was now measured from the footage using OSP Tracker's built-in protractor (manual tracking).
In addition to analyzing the overall decay to calculuate $Q$, $Q/n$, the number of oscillations (henceforth referred to as $K$) for the pendulum to decay by a factor of $\e ^ {-\pi/n}$, must be counted. 
$Q=nK,$ so $u(Q)=nu(K)$. Smaller $n$ result in a larger number of oscillations for which $\theta\approx\theta_{final}$ experimentally \mbox{$(\,\abs{\theta_{final}-\theta} \leq 2u(\theta))$}, increasing $u(K)$. Data from both $n=2$ and $n=3$ were collected, and the one with lower $u(Q)$ was used.


\section{Results and Data Analysis}
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight]{output-files/period vs amplitude.png}
    \caption{
        Graph of period vs. initial amplitude at release fitted to 3-term power series 
        $T(\theta_0) = T_0(1 + B\theta_0 + C\theta_0^2)$ (Wilson, 2025). 
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.4\textheight]{output-files/amplitude vs. time.png}
    \caption{
        Graph of $\theta$ at peak of oscillation vs. $t$ fitted to the exponential
