...
{\color{darkGreen}
This lab report presents results of experiments that examined predictions and dependencies between variables of the \textbf{damped harmonic oscillator} model of a pendulum. The equation
\begin{equation}
...
\end{equation}
would yield constant $T=T_0$, i.e. $B=0,\:C=0$ [1]. That is, $T$ is independent of $\theta_0$.
Analysis of collected data showed that $B=0$ (symmetric pendulum) but that a non-zero $C$ causes $T$ to increase quadratically with $|\theta_0|$. For $\abs{\theta_0}\leq0.25 \pm 0.03$, $T$ could reasonably be considered constant (agreeing with $T_0$ within error). This \textbf{small angle approximation} was used in all subsequent experiments. Note that two values $a \pm u(a)$, $b \pm u(b)$ (where $a<b$) \textbf{agree within error} if their error bars overlap ($a + u(a) \geq b - u(b))$. In the special case where $u(a)=u(b)$, $a$ and $b$ agree within error if $\abs{a-b}\leq 2u(a).$
...
\end{equation}
which quantifies the decay constant $\tau$ in proportion to $T$ [1].
$Q$ was also obtained by counting,
\begin{equation}
    Q=nK \label{q counted}
\end{equation}
where $K$ is the number of oscillations for $\theta_{\text{max}}$ to decay by a factor of $\e^{-\pi/n}$ for any chosen $n$. 
Equation \ref{max amplitude vs. time} fits the collected $\theta_{\text{max}}$ vs. $t$ data well but initially underestimates, then overestimates its true value. Considering error, the calculated $Q_{\text{fit}}=149\pm 2$ agreed with the $Q_2=154 \pm 10$ and $Q_3=165 \pm 9$ obtained by counting for $n=2$ and $n=3$ respectively.
...
\end{equation}
would yield $k=2$, $n=0.5$ [1], i.e. 
\begin{equation}
    T=2\sqrt{L} \label{period vs length bad}
\end{equation}
Collected data yielded fit values agreeing strongly with the predictions within error, but the units are incorrect. The theoretically derived equation [2]
\begin{equation}
    T=2\pi\sqrt{\frac{L}{g}} \label{period vs length}
\end{equation}
has correct units, and $k=\frac{2\pi}{\sqrt{g}}$ mostly agrees with $k=2$ within Experiment 3's error.



% EXPERIMENT 4
Experiment 4 examined the dependency of \textbf{$Q$ factor vs. length} using calculated $Q_{\text{fit}}$ values as in experiment 2. After fitting the data to many curves (Appendix \ref{trying curves}), the power law \hbox{$Q=kL^n$} yielded reasonable fit uncertainties and residuals with $k=380\pm\SI{20}{m^{-0.76}}$, $n=0.76 \pm 0.03$.
}
...
customized from \texttt{fit\_black\_box.py} [4] that parsed data, performed calculations, and generated best fit data (with uncertainties) and graphs {\color{darkGreen} using external packages \texttt{numpy}, \texttt{scipy}, and \texttt{matplotlib}.}
...
\subsection{Experiment 1: Examining Period vs. Initial Amplitude}
\vspace{-0.25cm}
...
When counting the number of oscillations $K$ (from Equation \ref{q counted}), $u(\theta)$ causes $\theta_{\text{max}}$ at multiple oscillations to agree with $\theta_{\text{final}}=\theta_0\e^{-\pi/n}$ within error, causing $u(K)$. $Q=nK$ (Equation~\ref{q counted}), so $u(Q)=nu(K)$. 
Selecting $n$ is a tradeoff because smaller $n$ can decrease $u(Q)$ but also decreases $\theta_{\text{final}}$, which can increase $u(K)$. Data from both $n=2$ and $n=3$ were collected.


\subsection{Experiment 3: Examining Period vs. Pendulum Length}
...
The pendulum was dropped from slightly higher than $\theta_0<0.25$ {\color{darkGreen}(the small angle approximation found in Experiment 1)} so it would stabilize after any potential human error (e.g. non-zero force) by the time it reached $\theta_0$.
...
\subsection{Experiment 4: Examining Q Factor vs. Length}
...
\begin{figure}[H]
    ...
    \caption{
        ...
        {\color{darkGreen}Most, but not all $T$ error bars overlap the best fit curve, so it is an accurate model with reasonable uncertainties}.}
...
\begin{figure}[H]
    ...
    \caption{
        ...
        {\color{darkGreen}Most, but not all error bars overlap the curve, so uncertainties are reasonable. The decay can reasonably be modelled with Equation \ref{max amplitude vs. time} but initially underestimates and eventually overestimates $\theta_{\text{max}}$.}
    }
...
\begin{figure}[H]
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \caption{
            ...
            $n=0.500 \pm 0.001$. 
            {\color{darkGreen}
            No error bars overlap the fitted curve, so $u(T)$ was likely underestimated.}
        }
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \caption{
            {\color{darkGreen}
            Graph of $T$ vs. $L$ on a logarithmic scale. Fitting to $\ln T = \ln k + n\ln L$ also yields $k=1.997\pm \SI{0.003}{s/m^{0.5}}$, 
...
As in the $T$ vs. $\theta_0$ analysis, $u(T)=\max(u_A(T), u_B(T))$ where $u_A(T)=$ standard error of the mean of 3 trials, ranging from $0.0006$ to $0.002\,$s, and $u_B(T)=1/\SI{120}{FPS}/\SI{10}{oscillations}=\SI{0.0008}{s}$. It is possible $u_A(T)$, and thus $u(T)$ was underestimated (see comments in Figure \ref{fig:period vs length}) due to insufficient trials. A combination of the metre stick's slight curve, its slight misalignment from the vertical, and parallax error cause at best $u(L)=\SI{0.005}{m}$.

{\color{darkGreen}
As predicted, exponent $n$ agrees with 0.5 within error and coefficient $k$ agrees with the predicted 2.000 within error. Equation \ref{period vs length bad} accurately states $T \propto \sqrt{L}$ but has incorrect units. Equation \ref{period vs length} has correct units, and since $k=\frac{2\pi}{\sqrt{g}}=2.006$ is only $3u(k)$ from the fitted $k$, the data reasonably matches theory.
}
...
\begin{figure}[H]
    \centering
        ...
        \caption{
            ...
            {\color{darkGreen}Most, but not all $Q$ error bars overlap with the curve, so it is an accurate model and uncertainties are reasonable.}
        }
...
\section{Conclusion}
\vspace{-0.22cm}
By analyzing experimental uncertainty, it is concluded that the period $T$ can be approximated as the small-angle period $T_0=1.528 \pm \SI{0.002}{s}$ for $\abs{\theta_0}\leq 0.25\pm0.03$ but grows quadratically for $\abs{\theta_0} > 0.25 \pm 0.03$ (magnitude $\abs{\theta_0}$ used as the pendulum was symmetric, $B=0$). The pendulum's $\theta_{\text{max}}$ per oscillation can be modelled with exponential decay $\theta_{\text{max}}(t)=\theta_0\e ^ {\tau/t}$, though the model initially underestimates, then overestimates $\theta_{\text{max}}$. 
$Q_{fit}=149\pm2$ calculated from $\tau$ and $T$ agrees with the $Q_2=150\pm10$ and $Q_3=165\pm9$ obtained by counting $K$ for $n=2$, $n=3$. 
{\color{darkGreen}
It was confirmed that $T \propto \sqrt{L}$. Best fit values matched equation \ref{period vs length bad}, but the theoretical equation \ref{period vs length} also matched the data and corrected units.
}
After trying multiple curves, $Q$ was found to depend on $L$, with $Q=kL^n$ and $k=380\pm \SI{20}{m^{-0.76}}$, $n=0.76 \pm 0.03$.

{\color{darkGreen}To improve results, more trials would be taken for the outlier in experiment 1 and for all points in experiment 3 as well (for more precise $u_A(T)$, since $u(T)$ was likely underestimated).}
...
Generative AI (ChatGPT) queried for basic questions about \texttt{matplotlib}, help debugging curve fitting errors, help fixing \LaTeX{} compilation errors, and general conventions in lab report formatting. No content was conceived, formulated, articulated, or revised by ChatGPT.
