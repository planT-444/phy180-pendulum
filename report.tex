...
\subsection{Period vs. Initial Amplitude}
...
As the pendulum could drift off plane with many oscillations, the mass was now hung from two strings of equal length to prevent motion normal to the desired plane. See Figure~\ref{fig:setup2}. The lens of the phone camera was normal to said plane and level with the centre of rotation to reduce parallax error, since max amplitude per oscillation was now measured using OSP Tracker (manual tracking). 
