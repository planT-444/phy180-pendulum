...
\end{center}
\pagebreak
Therefore, the small angle approximation is valid for $\abs{\theta_0}\leq 0.25 \pm 0.03$. According to Figure~\ref{fig:period vs amplitude}, the error bars are constant in this range and $u(T)=\SI{0.003}{s}$ is indeed valid for $\abs{\theta_0}\leq 0.25~ (u_A(T) < u_B(T))$. However, $T$ depends on $\theta_0$, growing quadratically and non-negligibly when $\abs{\theta_0}>0.25 \pm 0.03$.
