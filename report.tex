...
{\color{darkPurple}A longer length $L=58.0\pm 0.5\,\text{cm}$ was used for all trials, minimizing 
...
{\color{darkPurple} The shorter length $L=29.1 \pm 0.5\,\text{cm}$ was used for shorter $T$, increasing $\frac{u(T)}{T}$. However, since $\frac{u(T)}{T}\ll\frac{u(\tau)}{\tau}$, shorter $L$ does not increase $u(Q)=Q\cdot\max(\frac{u(\tau)}{\tau}, \frac{u(T)}{T})$.}
